...
class bttbWallaceb(bttbPage):
	...
	def title(self): return 'BTTB Wallace B. Wallace Awards'
	...
	def content(self):
		...
		wallaceList = self.__db.GetWallaceList()
		for name,displayName,year,description,submitTime in wallaceList:
			html += '<div class="outlinedTitle">'
			html += '%d - %s' % (year, displayName.replace(' ', '&nbsp;'))
			html += '</div>'
...
class testWallace(unittest.TestCase):
	def testDump(self):
		wallaceBPage = bttbWallaceb()
		print wallaceBPage.content()
