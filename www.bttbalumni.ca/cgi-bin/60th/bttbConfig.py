...
def AttendList():
	...
	in the database.
	"""
	return { 'Golf':3, 'Social':4, 'Homecoming':5, 'Brunch':6, 'Parade':1, 'Concert':2, 'Concert Practice':8 }

def VolunteerList():
	"""
	List of all events we requested volunteers for. Values are the event
	IDs in the database.
	"""
	return { 'Social':4, 'Homecoming':5, 'Brunch':6, 'Registration':7 }

def FullVolunteerList():
	"""
	List of all events we requested volunteers for. Values are the event
	IDs in the database.
	"""
	return { 'Social':4, 'Homecoming':5, 'Brunch':6, 'Registration':7, 'Friday':4, 'Saturday':5, 'Sunday':6 }
...
def TableList():
	...
	"""
	return [ "contact", "instruments", "alumni", "pages", "60thevents", "memories", "parade", "concert", "paid", "wallace", "attendance", "volunteers" ]
...
def ErrorMsg(msg,info):
	...
	"""
	if _errorsInHtml:
		return "<br><font size='+3' color='red'>ERROR:</font> %s %s <br> <p>Reload this page in a few seconds to try again.</p> <p>If problem persists contact the %s .</p></body>" % (msg, info, EmailLink("kpicott1\@cogeco.ca", "webmaster") )
	return "*** ERROR: %s\n           %s " % (msg, info)
...
def Warning(msg,info):
	...
	"""
	if _errorsInHtml:
		return "<br><font size='+3' color='red'>WARN:</font>%s %s<br> <p>Reload this page in a few seconds to try again.</p> <p>If problem persists contact the .</p>" % (msg, info, EmailLink("kpicott1\@cogeco.ca", "webmaster") )
	return "*** WARNING: %s\n             %s " % (msg, info)
...
def MailRon(subject='', text=''):
	"""
	Usage:
	MailRon('subject', 'Body of the mail')
	Send mail to Ron from the website.
	"""
	mailContent = "From: info@bttbalumni.ca\r\nTo: info@bttbalumni.ca\r\nSubject: %s\r\n\r\n%s" % (subject.replace('&nbsp;', ' '), text.replace('&nbsp;', ' '))
	try:
		if os.environ['REMOTE_ADDR'] != '127.0.0.1':
			mailServer = smtplib.SMTP('www.bttbalumni.ca')
			mailServer.sendmail('info@bttbalumni.ca', 'info@bttbalumni.ca', mailContent)
			mailServer.quit()
		else:
...
class testConfig(unittest.TestCase):
	def testError(self):
		...
		self.assertEqual( error, "<br><font size='+3' color='red'>ERROR:</font> hello world <br> <p>Reload this page in a few seconds to try again.</p> <p>If problem persists contact the %s .</p></body>" % EmailLink("kpicott1\@cogeco.ca", "webmaster") )

	def testEmail(self):
		mailto = EmailLink('kpicott@home.com')
		self.assertEqual( mailto, "<a href=\"javascript:makeEmail('__NOSPAM__kpicott','__NOSPAM__home.com')\" title=\"send email\">&#107;&#112;&#105;&#99;&#111;&#116;&#116;&#64;&#104;&#111;&#109;&#101;&#46;&#99;&#111;&#109;</a>" )
	...
	def testLinks(self):
		mail1 = EmailLink('kpicott@home.com', 'kpicott@home.com')
		mail2 = EmailLink('kpicott@home.com', 'me')
		mail3 = EmailLink('kpicott@home.com', 'let me know')
...
,			'abc 123 send:(kpicott@home.com) def 234 ': 'abc 123 ' + mail1 + ' def 234 '
,			'abc 123 send:(kpicott@home.com,me) def 234 ': 'abc 123 ' + mail2 + ' def 234 '
,			'abc 123 send:(kpicott@home.com,let me know) def 234 ': 'abc 123 ' + mail3 + ' def 234 '
			}
		for map in mapping:
